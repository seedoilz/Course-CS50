...
def transition_model(corpus, page, damping_factor):
    ...
    """
    distribution = {page_name: 0 for page_name in corpus}
    links = corpus[page]

    if len(links) == 0:
        for p in corpus:
            distribution[p] = 1 / len(links)
        return distribution
    ...
    no_linked_prob = (1 - damping_factor) / len(corpus)
    for p in corpus:
        distribution[p] += no_linked_prob
...
def sample_pagerank(corpus, damping_factor, n):
    ...
    for i in range(0, n - 1):
        ...
        for page, prob in trans_model.items():
            total_prob += trans_model[page]
            if total_prob >= rand_val:
                curr_page = page
                break
        visits[curr_page] += 1
